...
class SpeechPipe:
    def __init__(self, lang="en-US"):
        self.lang = lang # a BCP-47 language tag
        self.client = speech.SpeechClient()
        self.config = types.RecognitionConfig(encoding=enums.RecognitionConfig.AudioEncoding.LINEAR16,
            sample_rate_hertz=RATE, language_code=self.lang)
        self.streaming_config = types.StreamingRecognitionConfig(
            config=self.config,
            interim_results=True)
        rospy.init_node("asr_node", disable_signals=True)
        rospy.Service('asr', SpeechReco, self.service_callback)
        self.publisher_continuous()
        rospy.spin()
