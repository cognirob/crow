...
x = np.int16(np.zeros((1080,1920, 3)))
y = 0
t1=t2=0
n = 0
...
class CrowVision(Node):
  ...
  def mainfunn(img_raw_Color):
    ...
    #dil = 11
    #bg_blur = 3
    th = np.full((1080,1920, 3), mThres)


    # img_raw = cv2.cvtColor(img_raw_Color, cv2.COLOR_BGR2GRAY)
    # img_raw = cv2.cvtColor(img_raw_Color, cv2.COLOR_BGR2LAB)
    ...
    img_sub = cv2.absdiff(img2, x)
    # img_sub = cv2.absdiff(img2[..., 1:], x[..., 1:])
    # img_sub = cv2.absdiff(img2[..., 0:2], x[..., 0:2])
