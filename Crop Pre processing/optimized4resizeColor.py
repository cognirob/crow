...
class CrowVision(Node):
  ...
  #@jit#(nopython=True, parallel=False)
  def numm(img_raw_Color):
    th = np.full((540, 960, 3), 30)
    #img_raw = img_raw_Color.resize((400, 400))
    img_raw = cv2.cvtColor(img_raw_Color, cv2.COLOR_BGR2GRAY)
  ...
  def mainfunn(img_raw_Color):
    ...
    #dil = 11
    #bg_blur = 3
    th = np.full((540, 960, 3), mThres)

    img_raw = cv2.resize(img_raw_Color, (960,540), interpolation = cv2.INTER_AREA)
    #img_raw = img_raw_Color
    #img_raw = cv2.cvtColor(img_raw, cv2.COLOR_BGR2GRAY)
    #img_raw.resize((540, 960))
    #img_raw = cv2.resize(img_raw, (960,540), interpolation = cv2.INTER_AREA)
    ...
    for contour in contours:
      ...
      if area>800:
        ...
        hh= (img_raw_Color.shape[1]/x.shape[1])
        ww= (img_raw_Color.shape[0]/x.shape[0])
