...
y = 0
x = np.uint8(np.zeros((480,640,3)))
z = np.full((480,640,3), 255)
z = z.astype('uint8')

img_raw_sub = np.uint8(np.zeros((480,640,3)))
white = np.full((480,640,3), 255)
    ...
    for contour in contours:
      ...
      if area>800:
        ...
        if h>w:
          ...
          Bordered_img = cv2.copyMakeBorder(img2, xx, xx, xx, xx, borderType, None, [255,255,255])
          cropped = Bordered_img[xx+b : xx+b+h , xx+a-xx : xx+a+w+xx]
          #cv2.imwrite('Bordered_img.jpg', Bordered_img)
        else:
          yy = int(((w + gp * 2) * r - (h + gp * 2)) / 2)
          Bordered_img = cv2.copyMakeBorder(img2, yy, yy, yy, yy, borderType, None, [255,255,255])
