...
with db.onto:
    class update_object_position_node():
        ...
        def __init__(self):
            ...
            # self.onto = self._dbapi.get_onto()
            #
            # self.onto.__enter__()
            #
            # self.onto.__exit__()
        ...
        def handle_send_database_request(self, req):
            ...
            if self.database_locked:
                ...
                db.change_onto(new_onto_path)#TODO if error in changing onto -> success false, msg that could not be loaded
                res.received.success = True
                res.received.msg = 'Database received and loaded successfully'
                obj = db.onto.search(type=db.onto.RobotProgram)
                print(obj)
